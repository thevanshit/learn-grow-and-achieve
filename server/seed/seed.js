...
const insertBook = db.prepare('INSERT INTO books (id, batch_id, title, author, publisher, year, covers) VALUES (?, ?, ?, ?, ?, ?, ?)');
...
for (const b of batches) insertBatch.run(b.id, b.title, b.focus, b.calendar, b.weeks, b.color, b.icon);
for (const b of books) insertBook.run(b.id, b.batch_id, b.title, b.author, b.publisher, b.year, b.covers);
for (const w of weeks) insertWeek.run(w.id, w.batch_id, w.week, w.title, w.read, w.do);
for (const m of milestones) insertMilestone.run(m.id, m.title, m.description, m.books, m.week);
